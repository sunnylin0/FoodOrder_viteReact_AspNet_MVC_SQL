...
{
    /// <summary>
    /// FOOD_detailController FOOD_detail
     /// </summary>
    public class FOOD_detailController : BaseController
    {
        /// <summary>
        /// 資料列表
        /// </summary>
        ...
        public ActionResult Index(int page = 1, string searchText = "")
        {
            ...
            using (z_repodetail repos = new z_repodetail())
            {
        ...
        public ActionResult Detail(int id=0)
        {
            using (z_repodetail repos = new z_repodetail())
            {
                ...
                var model = repos.repo.ReadSingle(m => m.detailId == id);
                return View(model);
            }
        }
...
//                    {
//                        model.detailId = (int)attr.GetDefaultValue<z_metadetail>("detailId");
//                        model.orderId = (string)attr.GetDefaultValue<z_metadetail>("orderId");
//                        model.menuId = (string)attr.GetDefaultValue<z_metadetail>("menuId");
//                        model.menuName = (string)attr.GetDefaultValue<z_metadetail>("menuName");
//                        model.price = (int)attr.GetDefaultValue<z_metadetail>("price");
//                        model.subPrice = (int)attr.GetDefaultValue<z_metadetail>("subPrice");
//                        model.qty = (int)attr.GetDefaultValue<z_metadetail>("qty");
//                        model.remark = (string)attr.GetDefaultValue<z_metadetail>("remark");
//                    }
                }
                return View(model);
            }
        ...
        [HttpPost]
        [JwtAuthActionFilter()]
        public ActionResult CreateEdit(detail model)
        {
            if (!ModelState.IsValid)
            {
                TempData["ErrorMessage"] = ActionService.SetErrorMessage<z_metadetail>(ModelState);
                return View(model);
            }
            using (z_repodetail repos = new z_repodetail())
            {
