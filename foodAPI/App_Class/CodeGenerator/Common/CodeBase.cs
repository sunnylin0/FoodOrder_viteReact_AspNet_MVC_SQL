...
public class CodeBase : BaseClass
{
	...
	public string ApplicationName { get { return "bestERP"; } }
			...
			foreach (var item in myPropertyInfo)
			{
				...
				if (myMetadataInfo != null)
				{
					...
					if (metaName != null)
					{
						...
						ColumnsAttribute column = (ColumnsAttribute)Attribute.GetCustomAttribute(metaName, typeof(ColumnsAttribute));
...
";
	...
	public string GetDefaultStringValue(enDefaultValueType defaultType, string defaultValue)
	{
		...
		if (defaultType == enDefaultValueType.String_UserEMail) return UserService.UserEmail;
